 ...
 *
 * Same single-source-of-truth shape as the plain MVVM-XML sibling, but the VM
 * now depends on Domain use cases rather than the repository directly — so the
 * toggle-completion rule lives in [ToggleTaskCompletionUseCase], not here. The
 * derived `uiState` is still assembled with a [MediatorLiveData] (the LiveData
 * analog of StateFlow `combine`).
 */
class TaskListViewModel(
    ...
    fun toggleCompletion(task: TaskItem) {
        viewModelScope.launch {
            try {
                val saved = toggleTaskCompletion(task)
